...
class King:
    ...
    def __init__(self, screen, levels):
        ...
        self.jump_particle = King_Particle(
            "images\\particles\\jump_particle.png", 5, 1, 32
        )

        self.snow_jump_particle = King_Particle(
            "images\\particles\\snow_jump_particle.png", 4, 3, 36
        )
    ...
    def _add_gravity(self):
        ...
        self.angle, self.speed = self.physics.add_vectors(
            self.angle, self.speed, self.physics.gravity[0], self.physics.gravity[1]
        )
    ...
    def _collide_right(self, platform):
        ...
        if (
            ...
            and (
                platform.rect.top < rect.y < platform.rect.bottom
            ...
            and round(rect.x + rect.width - platform.rect.left, 4)
            <= math.ceil(math.sin(self.angle) * self.speed)
    ...
    def _collide_left(self, platform):
        ...
        if (
            ...
            and round(rect.x - platform.rect.right, 4)
            >= math.floor(math.sin(self.angle) * self.speed)
            # and round(math.sin(self.angle), 4) < 0
        ):
            return True
        else:
    ...
    def _collide_top(self, platform):
        ...
        if (
            ...
            and (
                platform.rect.left < self.rect_x < platform.rect.right
                or platform.rect.left
            ...
            and round(self.rect_y - platform.rect.bottom, 4)
            >= math.floor(-math.cos(self.angle) * self.speed)
    ...
    def _collide_bottom(self, platform):
        ...
        if (
            ...
            and round(self.rect_y + self.rect_height - platform.rect.top, 4)
            <= math.ceil(-math.cos(self.angle) * self.speed)
            # and round(-math.cos(self.angle), 4) > 0
        ):
            return True
        else:
    ...
    def _collide_slope_bottom(self, platform, rel_x):
        ...
        rel_y = platform.rect.bottom - (platform.rect.bottom - platform.rect.top) / (
            platform.rect.right - platform.rect.left
        ) * (rel_x)

        if self.rect_y + self.rect_height > rel_y > self.rect_y and (
            platform.rect.left < self.rect_x < platform.rect.right
    ...
    def _collide_slope_top(self, platform, rel_x):
        ...
        rel_y = platform.rect.top + (platform.rect.bottom - platform.rect.top) / (
            platform.rect.right - platform.rect.left
        ) * (rel_x)

        if self.rect_y < rel_y < self.rect_y + self.rect_height and (
            platform.rect.left < self.rect_x < platform.rect.right
            or platform.rect.left < self.rect_x + self.rect_width < platform.rect.right
            or self.rect_x < platform.rect.left < self.rect_x + self.rect_width
            or self.rect_x < platform.rect.right < self.rect_x + self.rect_width
                ...
                if platform.slope[1] > 0:
                    ...
                    if platform.slope[0] > 0:
                        ...
                        if self._collide_left(platform):
                            ...
                            self._collide_right(platform)
                            and self.rect_y + self.rect_height > platform.rect.bottom
                        ):
                            ...
                            while self._collide_slope_bottom(platform, rel_x):
                                ...
                                rel_x = (
                                    self.rect_x + self.rect_width - platform.rect.left
                                )
                ...
                if platform.slope[1] < 0:
                    ...
                    if platform.slope[0] < 0:
                        ...
                        if self._collide_left(platform):
                            ...
                            self._collide_right(platform)
                            and self.rect_y < platform.rect.top
                        ):
    ...
    def _update_vectors(self):
        ...
        if self.collideRamp and not self.collideBottom:
            ...
            self.angle, self.speed = self.physics.add_vectors(
                self.angle,
                self.speed,
                7 * math.pi / 4 * self.slope,
                self.physics.gravity[1],
            )
            self.angle, self.speed = self.physics.add_vectors(
                self.angle,
                self.speed,
                5 * math.pi / 4 * self.slope,
                self.physics.gravity[1],
            )
        ...
        if self.isFalling:
            ...
            if self.collideRight or self.collideLeft:
                ...
                if round(-math.cos(self.angle), 4) <= 0:
                    ...
                    self.angle = (
                        math.pi + (math.pi - self.angle) * self.angle_elasticity
                    )
        ...
        if self.isWalk:
            ...
            if self.walkCount <= 5:
                if not self.isHoldingBabe:
                    ...
                    self.current_image = self.sprites["ending"][
                        f"King_Walk1_{self.direction}"
                    ]
                ...
                if not self.isHoldingBabe:
                    ...
                    self.current_image = self.sprites["ending"][
                        f"King_MidWalk_{self.direction}"
                    ]
            ...
            if self.isSplat:
                ...
                and (self.idle_counter - self.idle_time) % self.idle_length == 0
            ):
                ...
                if self.isHoldingBabe:
                    self.current_image = self.sprites["ending"][
                        f"King_Hold_Babe_{self.direction}"
                    ]
            ...
            if not self.collided:
                ...
                if self.angle <= math.pi / 2 or self.angle >= 3 * math.pi / 2:
                    ...
                    if self.isHoldingBabe:
                        ...
                        self.current_image = self.sprites[self.direction][
                            "King_JumpFall"
                        ]
    ...
    def _update_audio2(self):
        ...
        if self.lastCollision:
            ...
            if self.isFalling:
                ...
                if (
                    any([self.collideTop, self.collideLeft, self.collideRight])
                    and not self.isWalk
                ):
