...
class Names:
    ...
    def _load_names(self):
        ...
        self.names[3] = "PML4 TABLE"

        self.names[4] = "PDP TABLE"

        self.names[5] = "DIR"

        self.names[6] = "PAGE TABLE"
    ...
    def blitme(self, screen):
        ...
        if self.blit_name:
            ...
            if self.blit_type:
                ...
                final_screen = pygame.Surface(
                    (text.get_width() + 100, text.get_height()), pygame.SRCALPHA
                )

                text_start, text_end = (
                    final_screen.get_width() - text.get_width()
                ) / 2, (
                    final_screen.get_width() - text.get_width()
                ) / 2 + text.get_width()
                ...
                pygame.draw.line(
                    final_screen,
                    (255, 255, 255),
                    (0, final_screen.get_height() / 2),
                    (text_start, final_screen.get_height() / 2),
                    1,
                )
                ...
                pygame.draw.line(
                    final_screen,
                    (255, 255, 255),
                    (text_end, final_screen.get_height() / 2),
                    (final_screen.get_width(), final_screen.get_height() / 2),
                    1,
                )
                ...
                final_screen.blit(
                    middle_screen, (0, 0), special_flags=pygame.BLEND_RGBA_MULT
                )

                screen.blit(
                    final_screen,
                    (
                        (screen.get_width() - final_screen.get_width()) / 2,
                        (screen.get_height() - final_screen.get_height()) / 2,
                    ),
                )
